...
export default function SetupAdmin({ onComplete }: Props) {
  ...
  return <main className="login-screen"><form className="login-card setup-card" onSubmit={submit}>
    ...
    <label>Senha<input required minLength={12} maxLength={128} type="password" autoComplete="new-password" value={form.password} onChange={event => setForm({ ...form, password: event.target.value })} /></label>
    <label>Confirmar senha<input required minLength={12} maxLength={128} type="password" autoComplete="new-password" value={form.confirmPassword} onChange={event => setForm({ ...form, confirmPassword: event.target.value })} /></label>
