...
export default function Customers({ customers, onAdd }: Props) {
  ...
  function submit(event: FormEvent) { event.preventDefault(); onAdd({ id: `c-${Date.now()}`, name, phone, email, createdAt: new Date().toISOString() }); setName(''); setPhone(''); setEmail(''); setShowForm(false) }
