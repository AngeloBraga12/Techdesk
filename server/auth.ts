...
function rateLimitFailedLogin(req: Request) { const now = Date.now(); const key = clientKey(req); const current = attempts.get(key); if (!current || current.resetAt <= now) { attempts.set(key, { count: 1, resetAt: now + 15 * 60 * 1000 }); return false }; current.count += 1; return current.count > 5 }
...
export function registerAuthRoutes(app: Express) {
  ...
  app.post('/api/auth/login', async (req, res, next) => { try { if (rateLimitFailedLogin(req)) return res.status(429).json({ error: 'RATE_LIMITED', message: 'Muitas tentativas de autenticação. Tente novamente mais tarde.' }); const email = normalizeEmail(req.body?.email); const password = req.body?.password; if (!validEmail(email) || typeof password !== 'string' || password.length > PASSWORD_MAX) return res.status(401).json({ error: 'INVALID_CREDENTIALS', message: 'E-mail ou senha inválidos.' }); const user = await prisma.user.findUnique({ where: { email } }); if (!user || !user.active || !(await verifyPassword(password, user.passwordHash))) { if (rateLimitFailedLogin(req)) return res.status(429).json({ error: 'RATE_LIMITED', message: 'Muitas tentativas de autenticação. Tente novamente mais tarde.' }); return res.status(401).json({ error: 'INVALID_CREDENTIALS', message: 'E-mail ou senha inválidos.' }) }; resetLoginAttempts(req); await prisma.session.deleteMany({ where: { expiresAt: { lt: new Date() } } }); const token = randomBytes(32).toString('base64url'); await prisma.session.create({ data: { tokenHash: hashSessionToken(token), userId: user.id, expiresAt: new Date(Date.now() + SESSION_TTL_MS) } }); setSessionCookie(res, token); return res.json({ user: publicUser(user) }) } catch (error) { return next(error) } })
