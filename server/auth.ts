...
async function verifyPassword(password: string, encoded: string) {
  ...
  const derived = await scrypt(password, Buffer.from(saltText, 'base64url'), expected.length, { N: Number(n), r: Number(r), p: Number(p), maxmem: SCRYPT_MAXMEM }) as Buffer
...
export async function authenticate(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    ...
    if (!session || session.expiresAt <= new Date() || !session.user.active) { if (session) await prisma.session.delete({ where: { id: session.id } }).catch(() => undefined); clearSessionCookie(res); return res.status(401).json({ error: 'UNAUTHORIZED', message: 'Sessão inválida ou expirada.' }) }
...
export function registerAuthRoutes(app: Express) {
  ...
  app.post('/api/auth/login', async (req, res, next) => {
    try {
      ...
      if (!user || !user.active || !(await verifyPassword(password, user.passwordHash))) { if (rateLimitFailedLogin(req)) return res.status(429).json({ error: 'RATE_LIMITED', message: 'Muitas tentativas de autenticação. Tente novamente mais tarde.' }); return res.status(401).json({ error: 'INVALID_CREDENTIALS', message: 'E-mail ou senha inválidos.' }) }
      resetLoginAttempts(req); await prisma.session.deleteMany({ where: { expiresAt: { lt: new Date() } } })
