...
function consumeRateBucket(map: Map<string, RateEntry>, key: string, now: number) {
...
export function registerAuthRoutes(app: Express) {
  app.post('/api/auth/register', async (req, res, next) => {
    try {
      ...
      if (!name || name.length > 120 || !validEmail(email) || !validPassword(password)) {
      ...
      const user = await prisma.$transaction(async tx => {
        ...
        return tx.user.create({
          data: {
            ...
            email,
            passwordHash,
            role: existingCount === 0 ? 'ADMIN' : 'TECHNICIAN',
          },
        })
      })
